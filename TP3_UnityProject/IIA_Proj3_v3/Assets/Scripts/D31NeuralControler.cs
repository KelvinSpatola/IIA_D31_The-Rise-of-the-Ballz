...
public class D31NeuralControler : MonoBehaviour
{
    ...
    public float GetScoreBlue()
    {
        ...
        /*
        float avgDistToBall = distanceToBall.Average();
        float minDistBallToAdv = distancefromBallToAdversaryGoal.Min();

        float fitness = ((1 / (1 + avgDistToBall)) * 1000) + ((1 / (1 + minDistBallToAdv)) * 5000) + (hitTheBall * 200) + (GoalsOnAdversaryGoal * 10000);
        Debug.Log("GENERATION = " + index + " || AVG DIST FIT = " + ((1 / (1 + avgDistToBall)) * 1000) + " || MIN DIST BALL TO ADV FIT = " + ((1 / (1 + minDistBallToAdv)) * 5000) + " || HIT THE BALL FIT = " + (hitTheBall * 5000) + " || GOALS FIT = " + (GoalsOnAdversaryGoal * 10000) + " || TOTAL FIT = " + fitness);
        */
        ...
        float avgDistToBall = distanceToBall.Average();
        float maxDistBallToMG = distancefromBallToMyGoal.Max();
        float minDistToMG = distanceToMyGoal.Min();

        float fitness = ((1 / (1 + avgDistToBall)) * 5000) + ((1 / (1 + maxDistBallToMG)) * 5000) + ((1 / (1 + minDistToMG)) * 5000) + (hitTheBall * 1000) - (GoalsOnMyGoal * 10000);
